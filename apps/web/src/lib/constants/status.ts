...
export const DOCUMENT_STATUS_CONFIG: Record<DocumentStatus, StatusConfig & { badgeClass: string }> = {
  PENDING: { label: 'En attente', variant: 'secondary', color: 'text-yellow-500', badgeClass: 'bg-yellow-500/20 text-yellow-400' },
  PROCESSING: { label: 'En cours', variant: 'secondary', color: 'text-blue-500', badgeClass: 'bg-blue-500/20 text-blue-400' },
  INDEXED: { label: 'Indexe', variant: 'default', color: 'text-green-500', badgeClass: 'bg-green-500/20 text-green-400' },
  FAILED: { label: 'Erreur', variant: 'destructive', color: 'text-red-500', badgeClass: 'bg-red-500/20 text-red-400' },
};
